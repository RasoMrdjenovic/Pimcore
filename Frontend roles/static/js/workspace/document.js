...
pimcore.registerNS("pimcore.plugin.myPlugin.workspace.document");
pimcore.plugin.myPlugin.workspace.document = Class.create({
    ...
    getPanel: function () {
        ...
        typesColumns.push({
           ...
           var documents = [];
           if(this.data.workspacesDocument){
               
               for (var i=0; i<this.data.workspacesDocument.length; i++){
                   var documentList = [];
                   documentList["path"] = this.data.workspacesDocument[i].path + this.data.workspacesDocument[i].key;
...
//           idProperty: 'id',
           fields: storeFields,
           data: documents
//            url: '/plugin/MyPlugin/admin/getworkspaces',
//           baseParams: { name : this.data.name }
       });
